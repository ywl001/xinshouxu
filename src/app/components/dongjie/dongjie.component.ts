...
import { Dongjie } from 'src/app/app-store/app-entities';
...
export class DongjieComponent extends ShouxuComponent implements OnInit {
  ...
  constructor(private cdr: ChangeDetectorRef, private clipboardService: ClipboardService) {
    super();
  }
  ...
  ngOnInit(): void {
    ...
    this.setCompanyValueByBankbin('freezeNumber','company')
    ...
    this.data$.subscribe((value:Dongjie)=>{
      console.log('dongjie shouxuitem change',value)
      if (value) {
  ...
  copyDocNumber() {
    let unit_1,unit_3;
    this.unit_1$.subscribe(val=>unit_1 = val);
    this.unit_3$.subscribe(val=>unit_3 = val);
    let str = this.freezeState.value === 'unfreeze' ? '解冻' : '冻';
    this.clipboardService.copy(`${unit_1}（${unit_3}）${str}财字[${this.year_start}]${this.docNumber}号`);
